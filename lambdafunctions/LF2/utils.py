...
def create_email_payload(restaurants, cuisine, number_of_people, dining_date):
    restaurants = get_restaurants_data_formatted(restaurants)
    subject = "Dining Concierge App: Recommendations"

    body_text = f"""
    Hello!
    Here are my {cuisine} restaurant suggestions for {number_of_people} people, for {dining_date}:
    ...
    <body>
      <h3>Hello!</h3>
      <p>Here are my {cuisine} restaurant suggestions for {number_of_people} people, for {dining_date}:</p>
      <ol>
